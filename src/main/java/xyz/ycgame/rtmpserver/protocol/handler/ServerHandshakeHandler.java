...
public class ServerHandshakeHandler extends ByteToMessageDecoder {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) {
        if (!partOneDone) {
            ...
            ctx.writeAndFlush(handshake.encodeServer0());
            ctx.writeAndFlush(handshake.encodeServer1());
            ctx.writeAndFlush(handshake.encodeServer2());
            partOneDone = true;
        }
        if (!handshakeDone) {
            ...
            handshakeDone = true;
            log.info("handshake done, rtmpe: {}", rtmpe);
            if (!rtmpe) {
    ...
    public static class RtmpHandshake {
        ...
        private static final byte[] RANDOM_CRUD = CodecUtils.fromHex(
        ...
        private static final byte[] DH_MODULUS_BYTES = CodecUtils.fromHex(
        ...
        private static byte[] digestHandshake(ByteBuf in, int digestOffset, byte[] key) {
            ...
            return CodecUtils.sha256(message, key);
        }
        ...
        private static int digestOffset(ByteBuf in, int validationType) {
            switch (validationType) {
                case 1:
                    return calculateOffset(in, 8, 728, 12);
                case 2:
                    return calculateOffset(in, 772, 728, 776);
                default:
        ...
        private static int publicKeyOffset(ByteBuf in, int validationType) {
            switch (validationType) {
                case 1:
                    return calculateOffset(in, 1532, 632, 772);
                case 2:
                    return calculateOffset(in, 768, 632, 8);
                default:
        ...
        private void initCiphers() {
            ...
            byte[] digestOut = CodecUtils.sha256(peerPublicKey, sharedSecret);
            byte[] digestIn = CodecUtils.sha256(ownPublicKey, sharedSecret);
            try {
                ...
                log.info("initialized encryption / decryption ciphers");
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        ...
        public ByteBuf encodeClient1() {
            ...
            log.info("using client version {}", CodecUtils.toHex(clientVersionToUse));
            if (validationType == 0) {
                out.copy();
                return out;
            }
            log.debug("creating client part 1, validation type: {}", validationType);
            initKeyPair();
        ...
        private void decodeServer0(ByteBuf in) {
            ...
            if (rtmpe && flag != 0x06) {
                log.warn("server does not support rtmpe! falling back to rtmp");
                rtmpe = false;
            }
        }
        ...
        private void decodeServer1(ByteBuf in) {
            ...
            log.debug("server time: {}, version: {}", CodecUtils.toHex(peerTime), CodecUtils.toHex(serverVersion));
            if (swfHash != null) {
                // swf verification
                byte[] key = new byte[DIGEST_SIZE];
                in.getBytes(HANDSHAKE_SIZE - DIGEST_SIZE, key);
                byte[] digest = CodecUtils.sha256(swfHash, key);
                ...
                log.info("calculated swf verification response: {}", CodecUtils.toHex(swfvBytes));
            }
            if (validationType == 0) {
                peerPartOne = in; // save for later
                return;
            }
            log.debug("processing server part 1, validation type: {}", validationType);
            ...
            if (!Arrays.equals(peerPartOneDigest, expected)) {
                ...
                log.warn("server part 1 validation failed for type {}, will try with type {}",
                        validationType,
                        altValidationType);
            ...
            log.info("server part 1 validation success");
        ...
        private void decodeServer2(ByteBuf in) {
            if (validationType == 0) {
                // TODO validate random echo
                return;
            }
            log.debug("processing server part 2 for validation");
            byte[] key = CodecUtils.sha256(ownPartOneDigest, SERVER_CONST_CRUD);
            ...
            log.info("server part 2 validation success");
        }
        ...
        public ByteBuf encodeClient2() {
            ...
            log.debug("creating client part 2 for validation");
            ByteBuf out = generateRandomHandshake();
            byte[] key = CodecUtils.sha256(peerPartOneDigest, CLIENT_CONST_CRUD);
            int digestOffset = HANDSHAKE_SIZE - DIGEST_SIZE;
        ...
        private void decodeClient0(ByteBuf in) {
            ...
            log.debug("client first byte {}, rtmpe: {}", CodecUtils.toHex(firstByte), rtmpe);
        }
        ...
        private boolean decodeClient1(ByteBuf in) {
            ...
            log.debug("client time: {}, version: {}", CodecUtils.toHex(peerTime), CodecUtils.toHex(peerVersion));
            validationType = getValidationTypeForClientVersion(peerVersion);
            if (validationType == 0) {
                peerPartOne = in; // save for later
                return true;
            }
            log.debug("processing client part 1 for validation type: {}", validationType);
            initKeyPair();
            int digestOffset = digestOffset(in, validationType);
            peerPartOneDigest = new byte[DIGEST_SIZE];
            in.getBytes(digestOffset, peerPartOneDigest);
            ...
            log.info("client part 1 validation success");
            int publicKeyOffset = publicKeyOffset(in, validationType);
            peerPublicKey = new byte[PUBLIC_KEY_SIZE];
            in.getBytes(publicKeyOffset, peerPublicKey);
            initCiphers();
        ...
        public ByteBuf encodeServer1() {
            ...
            log.debug("creating server part 1 for validation type: {}", validationType);
            int publicKeyOffset = publicKeyOffset(out, validationType);
            out.setBytes(publicKeyOffset, ownPublicKey);
            int digestOffset = digestOffset(out, validationType);
        ...
        public void decodeClient2(ByteBuf raw) {
            ...
            log.debug("processing client part 2 for validation");
            byte[] key = CodecUtils.sha256(ownPartOneDigest, CLIENT_CONST_CRUD);
            int digestOffset = HANDSHAKE_SIZE - DIGEST_SIZE;
            byte[] expected = digestHandshake(in, digestOffset, key);
            byte[] actual = new byte[DIGEST_SIZE];
            in.getBytes(digestOffset, actual);
            ...
            log.info("client part 2 validation success");
        }
        ...
        public ByteBuf encodeServer2() {
            ...
            log.debug("creating server part 2 for validation");
            ByteBuf out = generateRandomHandshake();
            byte[] key = CodecUtils.sha256(peerPartOneDigest, SERVER_CONST_CRUD);
            int digestOffset = HANDSHAKE_SIZE - DIGEST_SIZE;
            byte[] digest = digestHandshake(out, digestOffset, key);
            out.setBytes(digestOffset, digest);
            return out;
